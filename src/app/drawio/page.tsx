...
export default function Home() {
  ...
  const performSendMessage = async (displayContent: string, apiContent: string) => {
    ...
    try {
      ...
      const isRenderableSession = () => currentSessionRef.current === renderSessionId;
      ...
      const flushAgentPatch = () => {
        ...
        setMessages(prev => prev.map(message => {
          ...
          return {
            ...
            ...(patch.reasoning !== undefined ? { reasoning: nextReasoning } : {}),
      ...
      const finalizeStream = () => {
        ...
        if (streamAbortRef.current && streamAbortRef.current === currentController) {
          streamAbortRef.current = null;
        }
        setIsSending(false);
        setStreamPhaseSafely('');
      ...
      currentController = await agentApi.chatStream(
        ...
        (event: StreamEvent) => {
          ...
          switch (chunk.type) {
            ...
            case 'drawio_edge': {
              ...
              hasIncrementalContent = true;
              edgeCount++;
              setStreamProgressSafely(`添加连线 #${edgeCount}: ${chunk.label || chunk.source + '→' + chunk.target}`);
            ...
            case 'drawio_done': {
              ...
              if (isFinalStage) {
                  ...
                  if (drawioRef.current && isRenderableSession() && finalXml && finalXml.trim() !== '') {
                    try {
            ...
            case 'user': {
              ...
              if (chunk.content) {
                  ...
                  queueAgentPatch({ content: accumulatedContent, steps: cloneSteps() });
              }
              break;
            }
            ...
            case 'status': {
              ...
              if (chunk.content) {
                 ...
                 if (text !== '}' && text !== '{' && text !== ']' && text !== '[' && 
                     ...
                     const stepsChanged = updateStep(phase, currentPhaseLabel, chunk.content);
                     setStreamProgressSafely(text.substring(0, 50) + '...');
                     queueAgentPatch({ reasoning: accumulatedReasoning, ...(stepsChanged ? { steps: cloneSteps() } : {}) });
                 }
              }
              break;
            }
            ...
            case 'done': {
              ...
              if (nodeCount > 0 || receivedDrawioDone) {
                ...
                queueAgentPatch({ content: accumulatedContent, ...(stepsChanged ? { steps: cloneSteps() } : {}) });
        ...
        () => {
          ...
          if (!receivedDrawioDone && nodeCount > 0) {
              markAllStepsDone();
              const finalContent = (accumulatedContent ? accumulatedContent + '\n\n' : '') + `✅ 图表绘制完成！共 ${nodeCount} 个节点，${edgeCount} 条连线。`;
  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-hidden theme-bg-darkblue font-sans premium-grid">
      ...
      <div className="tool-header h-16 px-6 border-b border-slate-700/60 flex items-center justify-between shrink-0 z-40 relative bg-slate-900/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-100 tracking-tight">AI Bok 创作社区Sutmuch</h1>
            <p className="text-[11px] text-slate-400">Sutmuch Draw.io 创作台</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 rounded-full border border-slate-700 shadow-sm">
             ...
             <span className="text-xs font-semibold text-slate-300">{currentUser || 'Guest'}</span>
          </div>
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-1.5 bg-slate-800 border border-slate-700 text-slate-300 rounded-lg hover:bg-slate-700 hover:border-slate-600 hover:text-white transition-all text-sm font-medium shadow-sm active:scale-95"
          >
            ...
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                   ...
                   <span className="text-[10px] text-slate-500 font-medium leading-tight">Sutmuch Online</span>
                </div>
              </div>
            </div>
